        ...
        if valid:
            ...
            if mode == "stack":
                cv2.imshow(WINDOW_TITLE, np.hstack((
                    color_image, 
                    cv2.cvtColor(center_undistorted["right"][:,max_disp:], cv2.COLOR_GRAY2RGB),
                    disp_color
                    )))
            if mode == "overlay":
            ...
            if mode == "yolo":
                imgYolo = yoloTool.detect(color_image)
                cv2.imshow(WINDOW_TITLE, np.hstack((
                    cv2.cvtColor(center_undistorted["left"][:,max_disp:], cv2.COLOR_GRAY2RGB), 
                    imgYolo,
                    disp_color
                    )))
        key = cv2.waitKey(1)
